...
import { Search, Heart, X, Phone, Shield, MapPin, User, ChevronRight, UserPlus, Users, Link2, Plus, GraduationCap, Briefcase, Trash2 } from "lucide-react";
...
export default function DirectoryPage() {
  ...
  const handleDeleteMember = async (memberId: string, memberName: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!currentUser) return;
    if (!confirm(`⚠️ Are you sure you want to PERMANENTLY delete member "${memberName}"?\n\nThis action cannot be undone.`)) {
      return;
    }
    ...
    try {
      const res = await fetch(`/api/users/${memberId}`, {
        method: "DELETE",
        headers: {
          "x-caller-mobile": currentUser.mobileNumber,
        },
      });

      if (res.ok) {
        setUsers((prev) => prev.filter((u) => u._id !== memberId));
        alert(`🗑️ Member ${memberName} deleted successfully!`);
      } else {
        const data = await res.json();
            ...
            >
              ...
              <div className="flex items-center space-x-2 shrink-0 select-none">
                ...
                {(currentUser?.role === "admin" || currentUser?.role === "super-admin") && (
                  <button
                    type="button"
                    onClick={(e) => handleDeleteMember(contact._id, contact.name, e)}
                    className="w-8 h-8 rounded-full bg-rose-600 hover:bg-rose-700 text-white flex items-center justify-center shadow-xs active:scale-90 transition-transform cursor-pointer border-0"
